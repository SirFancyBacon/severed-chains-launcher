class_name AppConfig

const RSS_FEED_URL: String = "https://legendofdragoon.org/feed/"
const LOD_FAN_PAGE: String = "https://legendofdragoon.org/"
const LOD_FAN_DISCORD: String = "https://discord.gg/rQWXgK5"
const LOD_FAN_DISCORD_HELP: String = "https://discord.com/channels/307164262063669248/408013767151058946"
const GITHUB_API_URL: String = "https://api.github.com/repos/"

# --- Launcher Paths ---
const LAUNCHER_REPO: String = "sirfancybacon/severed-chains-launcher"
const ENGINE_REPO: String = "Legend-of-Dragoon-Modding/Severed-Chains"
const SC_STABLE_DIR: String = "severed-chains-stable"
const ISOS_DIR: String = SC_STABLE_DIR + "/isos"
const LAUNCH_CONF: String = SC_STABLE_DIR + "/launch.conf"
const TOKEN_PATH: String = MOD_DATA_DIR + "/github_api_token.txt"

# --- Mod Manager Paths ---
const OFFICIAL_MOD_LIST_URL: String = "https://raw.githubusercontent.com/SirFancyBacon/severed-chains-launcher/main/remote_mod_list.json"
const MOD_DATA_DIR: String = "mod_manager_data"
const MOD_CACHE_DIR: String =  MOD_DATA_DIR + "/cache"
const REMOTE_LIST_FILE: String = "remote_mod_list.json"
const CUSTOM_LIST_FILE: String = "custom_mod_list.json"
const MOD_STATE_FILE: String = "installed_mods.json"
const GAME_MODS_DIR: String = SC_STABLE_DIR + "/mods"
